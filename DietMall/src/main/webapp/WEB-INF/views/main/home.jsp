...
<body>
	...
	<section>
		...
		<div id="main_page">
			<div id="hd_slide">
				<img src="/resources/image/main/main_title.png" alt="메인 이미지">
			</div>
			...
			<div id="advert_one">
				<img src="/resources/image/main/main_avt1.png" alt="광고1">
			</div>

			<div id="sale_item">
				<h1>세일 상품</h1>
				<div class="sale_item_list">
				<%
					// 값 존재유무 확인
					List<ItemboardDTO> ib_dto_sale_list = new ArrayList<ItemboardDTO>();
					if(((ArrayList<ItemboardDTO>)request.getAttribute("ib_dto_sale_list")).size() != 0){
						// 존재함
						ib_dto_sale_list = (ArrayList<ItemboardDTO>)request.getAttribute("ib_dto_sale_list");
						for (int i=0; i<ib_dto_sale_list.size(); i++){
							ItemboardDTO ib_dto_sale = new ItemboardDTO();
							ib_dto_sale = ib_dto_sale_list.get(i);
					
				%>
		                <div class="sale_items_content"
		                onClick="location.href='/category/detail?item_bno=<%=ib_dto_sale.getItem_bno()%>&page_NowBno=1'">
		                    <!-- 상품 사진 -->
							<div class="items_img">
								<img src="/main/load-image?fileName=<%=ib_dto_sale.getItem_title_img()%>">
							</div>
							<!-- 상품 제목 -->
							<div class="items_title"><%=ib_dto_sale.getItem_name()%></div>
							<%
								// 세일 여부
								if (ib_dto_sale.getSale_yn().equals("Y")) {
							%>
							<!-- 상품 할인율, 원가, 할인가격 -->
							<div class="items_price_sale">
								<div class="sale_percent">
									<%
										int sale_price = ib_dto_sale.getOrigin_price() - (int)(ib_dto_sale.getOrigin_price()*(ib_dto_sale.getSale_percent()*0.01));
									%>
									<span class="percent"><%=ib_dto_sale.getSale_percent()%>%</span> <span class="percent_price">
										<fmt:formatNumber type="number" maxFractionDigits="3" value="<%=ib_dto_sale.getOrigin_price()%>" />
									</span>
								</div>
								<div class="sale_price">
									<span class="origin_price">
			...
			<div id="advert_two">
				<img src="/resources/image/main/main_avt2.png" alt="광고2">
			</div>
			...
			<div id="new_item">
				...
				<%
					...
					if(((ArrayList<ItemboardDTO>)request.getAttribute("ib_dto_new_list")).size() != 0){
						// 존재함
						ib_dto_new_list = (ArrayList<ItemboardDTO>)request.getAttribute("ib_dto_new_list");
						for (int i=0; i<ib_dto_new_list.size(); i++){
							ItemboardDTO ib_dto_new = new ItemboardDTO();
							ib_dto_new = ib_dto_new_list.get(i);
					
				%>
		                <div class="new_items_content"
		                onClick="location.href='/category/detail?item_bno=<%=ib_dto_new.getItem_bno()%>&page_NowBno=1'">
		                    <!-- 상품 사진 -->
							<div class="items_img">
								<img src="/main/load-image?fileName=<%=ib_dto_new.getItem_title_img()%>">
							</div>
							<!-- 상품 제목 -->
							<div class="items_title"><%=ib_dto_new.getItem_name()%></div>
							<%
								// 세일 여부
								if (ib_dto_new.getSale_yn().equals("Y")) {
							%>
							<!-- 상품 할인율, 원가, 할인가격 -->
							<div class="items_price_sale">
								<div class="sale_percent">
									<%
										int sale_price = ib_dto_new.getOrigin_price() - (int)(ib_dto_new.getOrigin_price()*(ib_dto_new.getSale_percent()*0.01));
									%>
									<span class="percent"><%=ib_dto_new.getSale_percent()%>%</span> <span class="percent_price">
										<fmt:formatNumber type="number" maxFractionDigits="3" value="<%=ib_dto_new.getOrigin_price()%>" />
									</span>
								</div>
								<div class="sale_price">
									<span class="origin_price">
